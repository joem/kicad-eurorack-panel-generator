...
describe Render do
  ...
  describe '#render_value' do
    ...
    it 'renders a the symbol name as a string when called on a Symbol' do
      value(@test_object.send(:render_value, :foo)).must_equal 'foo'
      value(@test_object.send(:render_value, :'bar')).must_equal 'bar'
    end

    it 'renders a string of elements with one space between when called on an Array' do
      value(@test_object.send(:render_value, [1, 1])).must_equal '1 1'
      value(@test_object.send(:render_value, [1, 1, 1])).must_equal '1 1 1'
      value(@test_object.send(:render_value, ['foo', 'bar'])).must_equal 'foo bar'
      value(@test_object.send(:render_value, [:baz, :bop])).must_equal 'baz bop'
    end

    #TODO: Test Hashes with #render_value
    it 'something something Hashes' do
      skip('Not sure what should actually happen in this case.') #FIXME
    end
  ...
  describe '#render_array' do
    ...
    it 'renders a string of elements with one space between when called on an Array' do
      value(@test_object.send(:render_array, [1, 1])).must_equal '1 1'
      value(@test_object.send(:render_array, [1, 1, 1])).must_equal '1 1 1'
      value(@test_object.send(:render_array, ['foo', 'bar'])).must_equal 'foo bar'
      value(@test_object.send(:render_array, [:baz, :bop])).must_equal 'baz bop'
    end

    #TODO: Test Strings with #render_array
    it 'something something Strings' do
      skip('Not sure what should actually happen in this case.') #FIXME
    end
  ...
  describe '#render_hash' do
    ...
    #TODO: Test Hashes with #render_hash
    it 'something something Hashes something something #render_hash' do
  ...
  describe '#check_for_characters_that_need_quoting' do
    # [' ', "\t", "\\t", "\n", "\\n", '(', ')', '%', '{', '}', '#']
  ...
  describe '#check_for_dash_anywhere_but_first' do
    #TODO: Write some tests!!!
    it 'does something' do
      skip('Test not implemented yet.') #FIXME
    end
  ...
  describe '#check_if_already_quoted' do
    #TODO: Write some tests!!!
    it 'does something' do
      skip('Test not implemented yet.') #FIXME
    end
  ...
  describe '#check_for_embedded_quote' do
    #TODO: Write some tests!!!
    it 'does something' do
      skip('Test not implemented yet.') #FIXME
    end
