    ...
    TEMPLATES = {
      ...
      "MountingHole:MountingHole_3.2mm_M3_DIN965" => <<~EOF.chomp,
        (module MountingHole:MountingHole_3.2mm_M3_DIN965 locked (layer F.Cu) (tedit 56D1B4CB) (tstamp <%= @tstamp %>)
          ...
          (attr virtual)
          (fp_text reference <%= @reference %> (at 0 -3.8) (layer F.SilkS) hide
          ...
          (pad 1 np_thru_hole <%= @extra_options[:hole_shape].to_s %> (at 0 0) (size <%= (@extra_options[:hole_shape].to_s == 'oval') ? '5' : '3.2' %> 3.2) (drill <%= (@extra_options[:hole_shape].to_s == 'oval') ? 'oval 5 ' : '' %>3.2) (layers *.Cu *.Mask))
        )
        EOF
    }
...
#          (pad 1 np_thru_hole <%= @extra_options[:hole_shape].to_s %> (at 0 0) (size 3.2 3.2) (drill <%= (@extra_options[:hole_shape].to_s == 'oval') ? 'oval 5 ' : '' %>3.2) (layers *.Cu *.Mask))
