...
module KicadPcb
  ...
  #TODO: Add mounting holes possibility
  #       - add them as modules
  #       - be sure to increase the modules count in general (@setting_modules)
  class Pcb
    ...
    def setup
      ...
      (setup
      #{KicadPcb.indent(item_list, 2)}
      #{KicadPcb.indent(pcbplotparams,2)}
      )
      EOF_SETUP
    end
    ...
    def edge_cuts
      ...
      (gr_line (start #{KicadPcb.num_to_s(x1)} #{KicadPcb.num_to_s(y1)}) (end #{KicadPcb.num_to_s(x1)} #{KicadPcb.num_to_s(y2)}) (layer Edge.Cuts) (width 0.05) (tstamp #{KicadPcb.current_tstamp}))
      (gr_line (start #{KicadPcb.num_to_s(x2)} #{KicadPcb.num_to_s(y1)}) (end #{KicadPcb.num_to_s(x1)} #{KicadPcb.num_to_s(y1)}) (layer Edge.Cuts) (width 0.05))
      (gr_line (start #{KicadPcb.num_to_s(x2)} #{KicadPcb.num_to_s(y2)}) (end #{KicadPcb.num_to_s(x2)} #{KicadPcb.num_to_s(y1)}) (layer Edge.Cuts) (width 0.05))
      (gr_line (start #{KicadPcb.num_to_s(x1)} #{KicadPcb.num_to_s(y2)}) (end #{KicadPcb.num_to_s(x2)} #{KicadPcb.num_to_s(y2)}) (layer Edge.Cuts) (width 0.05))
      EOF_EDGE_CUTS
    end
    ...
    def output
      ...
      #{KicadPcb.indent(general,2)}

      #{KicadPcb.indent(page,2)}
      #{KicadPcb.indent(layers,2)}

      #{KicadPcb.indent(setup,2)}

      #{KicadPcb.indent(net,2)}

      #{KicadPcb.indent(net_class,2)}
      #{KicadPcb.indent(render_module_list,2)}
      #{KicadPcb.indent(edge_cuts,2)}
